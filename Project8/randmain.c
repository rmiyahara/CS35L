...
main (int argc, char **argv)
{
  ...
  if (rdrand_supported ())
    {
      initialize = dlopen("randlibhw.so", RTLD_LAZY);
      if (initialize == NULL)
      {
      ...
      if (rand64 == NULL)
      {
        fprintf(stderr, "Error obtaining address of randlib. (Hardware)");
        exit(1);
      }
    }
  else
    {
      initialize = dlopen("randlibsw.so", RTLD_LAZY);
      if (initialize == NULL)
      {
      ...
      if (rand64 == NULL)
      {
        fprintf(stderr, "Error obtaining address of randlib. (Software)");
        exit(1);
      }
    }
